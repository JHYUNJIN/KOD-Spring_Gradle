...
import com.koreait.kod.biz.coupon.CouponCategoryDTO;
import com.koreait.kod.biz.coupon.CouponCategoryService;
...
public class IssueCouponByGrade {
	...
	public String issueCouponByGrade(@RequestParam("categoryID") int categoryID,
			CouponDTO couponDTO,
			CouponStatusDTO couponStatusDTO,
			CouponCategoryDTO couponCategoryDTO,
			MemberDTO memberDTO,
			Model model,
			HttpSession session) {
		System.out.println("[로그:정현진] 쿠폰발행 컨트롤러 들어옴");
		System.out.println("[로그:정현진] 쿠폰 카테고리 : "+categoryID);
...
//		}
		...
		couponCategoryDTO.setCategoryID(categoryID);
		if(!couponCategoryService.insert(couponCategoryDTO)) {
			System.out.println("[로그:정현진] 쿠폰카테고리 등록 오류");
			model.addAttribute("msg", "쿠폰카테고리 등록 오류");
			return "common/goback";
		}
...
//			couponStatusDTO.setCouponExpireDate(couponExpireDate);
